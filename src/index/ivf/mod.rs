...
impl IvfBuilder {
	...
	pub fn build(&self, vectors: &[f32], _ids: &[u32]) -> Result<IvfIndex, DendraError> {
		...
		let nlist = self.nlist.unwrap_or(inferred_nlist).min(vector_count.max(1));
		...
		index.train_centroids_kmeanspp(vectors, self.seed, self.train_sample_size, self.train_iters)?;
...
impl IvfIndex {
	...
	pub fn candidate_lookups<'a>(&'a self, candidate: &Candidate) -> Result<&'a [u32], DendraError> {
	...
	fn shuffle_postings_in_lists(&mut self, seed: u64) {
		for list_idx in 0..self.nlist {
			...
			let mut state = seed
				^ ((list_idx as u64).wrapping_mul(0x9E3779B97F4A7C15))
				^ 0xA24BAED4963EE407;
